...
fontsw() {
    ...
    if grep -q '"editor.fontFamily": "JetBrainsMono-Bold"' "$settings_file"; then
        ...
        tput cuu1
        sed -i.bak 's/"editor.fontFamily": "JetBrainsMono-Bold"/"editor.fontFamily": "JetBrains Mono"/' "$settings_file"
        echo -e "${INVERTED} ☡ FONT Changed to JetBrains Mono ${RESET}"
    else
        sed -i.bak 's/"editor.fontFamily": "JetBrains Mono"/"editor.fontFamily": "JetBrainsMono-Bold"/' "$settings_file"
